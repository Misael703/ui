> Archivado 2026-09-02 — todo lo listado shipeó entre v1.68.5 y v1.84.0; ver CHANGELOG.
...
- [x] `index.css:1863` — `--color-grey-200` → `--color-gray-200` (dead NumberInput stepper hover). ✓
- [x] `Button.tsx:72,88` — loading spinner hardcoded `spinner--inverse` (white) invisible on outline/ghost/subtle/link/warning. Make ring inherit `currentColor` (drop `--inverse`), or pick spinner modifier from `variant`. Add a story/test that renders a loading Button per variant. ✓
- [x] `Button.tsx:61-77` + `index.css` — `<Button asChild disabled/loading>` has no effect: add `.btn[aria-disabled="true"]{opacity:.45;pointer-events:none}` and guard the merged `onClick`. ✓
- [x] `AuthSplit.tsx:30,118` — mobile form stuck at 50%. Add a root class and collapse container to `grid-template-columns:1fr` inside the `@media (max-width:768px)`. ✓
- [x] `Icons.tsx:105-106` — `Tool` === `Wrench` (byte-identical). Give `Tool` the lucide crossed wrench+screwdriver glyph, or make it an explicit named alias. ✓
- [x] `DataTablePage.tsx:118-119` — dual sort; column-header sort is dead. Pick one model (drive `filtered` from `sort` and drop `SortDropdown`, OR remove `sort`/`onSortChange`). ✓
- [x] `CheckoutSummary.tsx:59-63` — derive `iva = round(subtotal*0.19)` and total from line items (mirror InvoiceDocument). ✓

## Batch 2 — Ban cleanup (the remaining slop tells) → patch bump

- [x] `index.css:1508` — Toast: drop `border-left-width:4px` + per-variant `border-left-color`; carry variant via the already-colored icon + optional 1px `border-color`/tint. ✓ (2 reviewers)
- [x] `index.css:2936` — AppShell active-nav: consider dropping the 3px `::before` stripe in favor of the tinted `is-active` background already on the selector. (Borderline — confirm intent.)
- [x] Em-dash sweep: `AvailabilityCalendar.tsx:49`, `RouteMap.tsx:67` titles → `·`. Null-sentinels in Audit/DeliveryTimeline/Commerce CompareTable → `undefined` + rendered "Sin dato".
