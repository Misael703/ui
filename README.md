...
Para los `items.icon` del nav (especialmente con el sidebar colapsado al rail, donde sólo se ve el icono), usa el set ya empaquetado en `@misael703/ui` — no es necesario dibujar SVGs a mano: `Home`, `List`, `Wallet`, `History`, `ShoppingCart`, `CreditCard`, `Settings`, `Users`, etc. Catálogo completo en la sección [Iconos](#iconos) más abajo.
