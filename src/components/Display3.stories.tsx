...
export const TimelineDemo: StoryObj = {
  ...
  render: () => (
    <Timeline style={{ maxWidth: 480 }}>
      <TimelineItem tone="success" icon={<CheckCircle size={14} />} title="Pedido creado" meta="2026-04-29 09:14 · Satoru Gojo">
  ...
  render: () => {
    ...
    return (
      <Timeline style={{ maxWidth: 480 }}>
        ...
        <TimelineItem state="current" tone="info"    icon={<Num n={3} />} title="Enviar" meta="En curso" />
    ...
    <Timeline style={{ maxWidth: 540 }}>
      ...
      <TimelineItem state="done" tone="success" icon={<Truck size={14} />} title="Envío 1/3 enviado" meta="14:30">
        <div style={{ border: '1px solid var(--border-default)', borderRadius: 8, padding: 10, fontSize: 12 }}>
          <div><strong>Guía:</strong> #DG-78422 · Courier Andes</div>
  ...
  render: () => {
    ...
    return (
      <div style={{ maxWidth: 720 }}>
        <Calendar
          events={[
            { date: today, label: 'Entrega 1042', tone: 'info' },
...
interface OrderCycleArgs { events: number; payload: boolean; types: boolean; density: 'default' | 'compact'; completed: boolean }

/**
 * **Playground · ciclo de un pedido.** Las cuatro capas que el kit ofrece
 * para contar el ciclo completo de un recurso, combinadas:
 *
 *  - **Anchors fijos** (`variant="milestone"`) arriba y abajo — "creado"
 *    siempre `state="done"`; "entregado" morphea entre `pending` y `done`.
 *  - **Eventos operativos** (markers 24×24) en el medio — `state` refleja
 *    avance (`done` / `current` / `pending`), `tone` lo refuerza con color.
 *  - **Tipo de evento** via `right={<Badge>…</Badge>}` — se escanea el "qué"
 *    sin leer el "qué pasó".
 *  - **Payload estructurado** en `children` para los items que cargan
 *    detalle — los que no tienen, no lo muestran.
 *
 * La fórmula escala de 0 a N eventos sin cambios de código: con `events: 0`
 * quedan los dos anchors ("se creó, falta todo"); `completed` cierra la
 * historia (todo `done`, anchor inferior `done`). `density="compact"` es el
 * mismo patrón para la card de una lista.
 */
export const CicloDePedidoPlayground: StoryObj<OrderCycleArgs> = {
  name: 'Playground · ciclo de un pedido',
  ...
  render: (a) => {
      ...
      { title: 'Envío 1/3 enviado', meta: '25-05-2026 14:30 · Bodega norte', doneMeta: '25-05-2026 14:30 · Bodega norte', icon: <Truck size={size} />, type: <Badge variant="primary">envío</Badge>, state: 'done', tone: 'success',
        payload: (
          <div style={{ marginTop: 6, padding: '8px 12px', border: '1px solid var(--border-default)', borderRadius: 8, fontSize: 12, color: 'var(--fg-muted)', display: 'grid', gap: 2 }}>
            <div><span style={{ color: 'var(--fg-subtle)' }}>Guía:</span> DG-78422 · Courier Andes</div>
    ...
    return (
      <Timeline density={a.density === 'compact' ? 'compact' : undefined} style={{ maxWidth: 580 }}>
        ...
        <TimelineItem variant="milestone" state={a.completed ? 'done' : 'pending'} tone="success" icon={<CheckCircle size={anchor} />} title="Pedido entregado" meta={a.completed ? '27-05-2026 16:45' : 'Pendiente'} />
      </Timeline>
    );
  },
};
